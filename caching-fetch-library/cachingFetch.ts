...
// Global cache object to store fetched data
...
export const useCachingFetch: UseCachingFetch = (url) => {
  // Use state initialization function to handle both SSR and CSR cases
  const [state, setState] = React.useState(() => {
    if (cache[url]) {
      return { isLoading: false, data: cache[url], error: null };
    }
  ...
  React.useEffect(() => {
    if (!cache[url]) {
      fetch(url)
        ...
        .then((jsonData) => {
          cache[url] = jsonData;
          setState({ isLoading: false, data: jsonData, error: null });
        })
        .catch((fetchError) => {
          setState({ isLoading: false, data: null, error: fetchError });
        });
    }
  }, [url]);
...
export const preloadCachingFetch = async (url: string): Promise<void> => {
  if (!cache[url]) {
    ...
    cache[url] = jsonData; // Store in cache
  }
};
...
// 3.2 Initialize the cache from a serialized cache string
export const initializeCache = (serializedCache: string): void => {
  cache = JSON.parse(serializedCache);
};

// Utility to clear the cache (if needed)
